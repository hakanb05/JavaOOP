...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Welkom bij het tentamen Artiestenboeker: <Naam> <Klas> <Studentnummer>\n");
...
//
    ...
    public static void printToneelgezelschappenInJaar(int jaar) {
        for (int i = 0; i < artiesten.size(); i++) {
            if (artiesten.get(i) instanceof Toneelgezelschap) {
                if (((Toneelgezelschap) artiesten.get(i)).getJaarOprichting() <= jaar) {
                    System.out.println(artiesten.get(i));
                }
            }
        }
    }
    ...
    public static void printArtiestenPerGenre() {
        artiesten.sort(Artiest::compareTo);
        for (int i = 0; i < artiesten.size(); i++) {
            System.out.println(artiesten.get(i));
        }
    }
    ...
    public static void boekArtiest(String artiestnaam, LocalDate datum) {
        for (int i = 0; i < artiesten.size(); i++) {
            if (artiesten.get(i).krijgnaam().contains(artiestnaam)) {
                if (artiesten.get(i).boek(datum)) {
                    System.out.println(artiesten.get(i).krijgnaam() + " is geboekt op " + datum);
                } else {
                    System.out.println(artiesten.get(i).krijgnaam() + " is al geboekt op " + datum);
                }
            }
        }
    }
